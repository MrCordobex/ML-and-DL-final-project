...
\begin{itemize}
    \item \textbf{Sección 2:} Descripción del dataset de Kaggle de enfermedades cutáneas
...
% 2. DATASET
% ============================================
\section{Dataset: Enfermedades Cutáneas (Kaggle)}

Para este proyecto se ha seleccionado un dataset público obtenido de \textbf{Kaggle}, que contiene imágenes de enfermedades cutáneas junto con metadatos tabulares del paciente.
...
\bottomrule
\end{tabular}
\caption{Clases diagnósticas del dataset de enfermedades cutáneas}
\end{table}
...
Esta sección describe la implementación de modelos de Deep Learning para clasificación de imágenes de enfermedades cutáneas utilizando \textbf{Transfer Learning} con arquitecturas pre-entrenadas en ImageNet.
...
% ========== 3.2.8 EXPERIMENTOS REALIZADOS ==========
\subsubsection{Experimentos Realizados}% (Sección de Experimentos eliminada para focalizar en la solución final)

% ========== 3.2.9 RESULTADOS MODELOS INDIVIDUALES ==========
\subsubsection{Resultados de Componentes Individuales}
...
\bottomrule
\end{tabular}
\caption{Resultados individuales de cada arquitectura base en el conjunto de test. Los modelos seleccionados para el ensemble final (ResNet-18, DenseNet-121 y YOLO26x) demostraron un mejor equilibrio entre capacidad y generalización que sus variantes más profundas.}
\label{tab:resultados_individuales}
\end{table}

% (Sección de Análisis de Arquitecturas Profundas eliminada)

\subsubsection{Análisis de Convergencia y Entrenamiento}

Las curvas de aprendizaje mostradas en las Figuras \ref{fig:curvas_resnet}, \ref{fig:curvas_densenet} y \ref{fig:curvas_yolo} revelan dinámicas de entrenamiento distintivas para cada arquitectura:

\begin{itemize}
    \item \textbf{ResNet-18 (Fig. \ref{fig:curvas_resnet}):} Muestra una convergencia estable pero lenta. La pérdida de validación se estanca alrededor de la época 15, indicando el límite de capacidad de extracción de features con los datos disponibles. No se observan picos drásticos de divergencia, lo que sugiere que el learning rate de $10^{-4}$ fue adecuado.
    
    \item \textbf{DenseNet-121 (Fig. \ref{fig:curvas_densenet}):} Gracias a la reutilización de características, logra reducir la pérdida de entrenamiento más rápidamente que ResNet. Sin embargo, se observa una mayor oscilación en la pérdida de validación, un comportamiento típico de las conexiones densas que propagan gradientes intensos. El \textit{Early Stopping} actuó correctamente para evitar el sobreajuste al detener el entrenamiento cuando la métrica de validación dejó de mejorar.
    
    \item \textbf{YOLO26x-cls (Fig. \ref{fig:curvas_yolo}):} Presenta la convergencia más agresiva y eficiente. La pérdida cae drásticamente en las primeras 5 épocas, demostrando la eficacia del backbone CSPNet para capturar patrones visuales robustos rápidamente. Es notable cómo logra mantener la pérdida de validación baja por más tiempo antes de sobreajustarse, lo que explica su superioridad en la métrica final de F1-Macro.
\end{itemize}
...
El ensemble final combina tres modelos optimizados:
...
\begin{lstlisting}[caption={Pseudocódigo de Inferencia del Ensemble}]
...
\subsubsection{Resultados del Ensemble Final}
...
La Figura \ref{fig:confusion_ensemble} ilustra la distribución de predicciones del ensemble. Se destacan varios patrones clave:

\begin{itemize}
    \item \textbf{Diagonal Dominante:} La concentración de valores en la diagonal principal confirma la alta capacidad global del modelo (Accuracy 62.7\%).
    
    \item \textbf{Éxito en Melanoma (MEL):} La fila correspondiente a MEL muestra una precisión perfecta (100\%), lo que es un hito crítico. El modelo demostró ser extremadamente conservador y preciso al clasificar lesiones malignas graves, evitando falsos positivos que causarían alarmas innecesarias, aunque su recall (71\%) indica que aún pierde algunos casos (falsos negativos), clasificándolos erróneamente como NEV (Nevus) o BCC.
    
    \item \textbf{Confusión NEV vs MEL:} Existe cierta confusión entre Nevus (benigno) y Melanoma (maligno), lo cual es esperable dada su similitud visual. Sin embargo, el ensemble prioriza correctamente la especificidad.
    
    \item \textbf{Clases Difíciles:} SCC (Carcinoma de Células Escamosas) presenta la mayor dispersión, confundiéndose frecuentemente con BCC y ACK. Esto sugiere que estas clases comparten características morfológicas (como la hiperqueratosis) que son difíciles de desenmarañar con la resolución actual.
\end{itemize}
...
\multirow{2}{*}{Híbrido}
& Fusión Temprana & --- & --- & --- & --- \\
& \textbf{Ensemble CNN (Final)} & \textbf{62.7\%} & \textbf{0.65} & \textbf{0.55} & \textbf{0.83} \\
\bottomrule
\end{tabular}
...
% (Tabla de comparación de pruebas eliminada)
...
\begin{itemize}
    \item Aumentar el dataset con más imágenes de Kaggle o fuentes externas
